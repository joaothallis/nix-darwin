...

{
  ...
  programs.fish = {
    ...
    shellAliases = {
      ...
      gc- = "git checkout -";
      gb = "git checkout -b";
      gcai = "git --no-pager diff HEAD | mods --model gemini-1.5-pro-latest 'write a commit message for this patch. also write the long commit message. use semantic commits. break the lines at 80 chars' | tr -s ' ' >.git/gcai; git commit -a -F .git/gcai -e";
